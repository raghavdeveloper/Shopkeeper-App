...
class AuthProvider extends ChangeNotifier {
  ...
  //register shop using email

  Future<UserCredential> registerShop(email, password) async {
    this.email = email;
    notifyListeners();
  ...
  //login
  Future<UserCredential> loginShop(email, password) async {
    this.email = email;
    notifyListeners();
    UserCredential userCredential;
    try {
  ...
  Future<void> saveShopDataToDb(
      {String url, String shopName, String mobile, String dialog}) {
    User user = FirebaseAuth.instance.currentUser;
    DocumentReference _shops =
        FirebaseFirestore.instance.collection('shops').doc(user.uid);
    _shops.set({
      'uid': user.uid,
      'shopName': shopName,
      'mobile': mobile,
      ...
      'rating': 0.00, //will use later
      'totalRating': 0, //will use later
      'isTopPicked': true, //will use later
      'imageUrl': url,
      'accVerified': true,
    });
    return null;
